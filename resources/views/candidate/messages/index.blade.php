...
<div style="max-width:720px;">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:28px;">
        <div>
            ...
            <p style="font-size:13px;">Conversations with employers about your applications.</p>
        </div>
        @if($totalUnread > 0)
            <span class="badge badge-danger">{{ $totalUnread }} unread</span>
        @endif
    </div>
    ...
    @if($applications->isEmpty())
        <div style="text-align:center;padding:64px 24px;background:var(--bg-surface);
            ...
            <p style="font-size:14px;">Messages will appear here after employers contact you.</p>
        </div>
    @else
        <div style="display:flex;flex-direction:column;gap:10px;">
            @foreach($applications as $application)
                ...
                <a href="{{ route('candidate.messages.show', $application) }}"
                    ...
                    <div class="card" style="padding:20px;{{ $unread ? 'border-color:rgba(167,139,250,0.4)' : '' }}">
                        <div style="display:flex;align-items:center;gap:14px;">
                            <div style="width:44px;height:44px;border-radius:50%;background:var(--bg-elevated);
                                        ...
                                        font-weight:700;color:var(--spark);flex-shrink:0;">
                            ...
                            @if($unread)
                                <div style="width:10px;height:10px;border-radius:50%;
                                            background:var(--accent-bright);flex-shrink:0;"></div>
                            @endif
                        </div>
                    </div>
                </a>
