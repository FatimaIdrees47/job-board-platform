...
<div style="max-width:720px;">
    ...
    <div class="card" style="padding:20px;margin-bottom:16px;">
        <div style="display:flex;align-items:center;gap:12px;">
            <div style="width:44px;height:44px;border-radius:50%;background:var(--bg-elevated);
                        ...
                        font-weight:700;color:var(--spark);">
                {{ strtoupper(substr($application->job->employer->company_name, 0, 2)) }}
            </div>
            <div>
                <div style="font-size:15px;font-weight:600;">{{ $application->job->employer->company_name }}</div>
    ...
    <div class="card" style="padding:20px;">
        <form method="POST" action="{{ route('candidate.messages.send', $application) }}">
            @csrf
